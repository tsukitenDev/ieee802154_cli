// Source code based on https://github.com/espressif/esp-idf/blob/master/examples/ieee802154/ieee802154_cli/components/cmd_ieee802154/ieee802154_cmd.c
...
static void esp_ieee802154_frame_print(const uint8_t *frame)
{
    ...
    uint8_t frame_type = frame[1] & 0x07;
    uint8_t sec_enb = (frame[1] >> 3) & 0x01;
    uint8_t pending = (frame[1] >> 4) & 0x01;
    uint8_t ack_req = (frame[1] >> 5) & 0x01;
    uint8_t panid_comp = (frame[1] >> 6) & 0x01;
    
    uint8_t seq_comp     = frame[2]        & 0x01;
    uint8_t ielist       = (frame[2] >> 1) & 0x01;
    uint16_t dst_addr_type = (frame[2] >> 2) & 0x03;
    uint16_t frame_ver    = (frame[2] >> 4) & 0x03;
    uint16_t src_addr_type = (frame[2] >> 6) & 0x03;
    ESP_EARLY_LOGI(TAG, "Frame Type      : %d   Seq Num Supress: %d", frame_type, seq_comp);
    ESP_EARLY_LOGI(TAG, "Security Enabled: %d   IE Present     : %d", sec_enb, ielist);
    ESP_EARLY_LOGI(TAG, "Frame Pending   : %d   Dst Addr Mode  : %d", pending, dst_addr_type);
    ESP_EARLY_LOGI(TAG, "Ack Request     : %d   Frame Version  : %d", ack_req, frame_ver);
    ESP_EARLY_LOGI(TAG, "PAN ID Compress : %d   Src Addr Mode  : %d", panid_comp, src_addr_type);
    ESP_EARLY_LOGI(TAG, "Sequence Number : %d", frame[3]);
    ...
    dst_panid = frame[idx] | ((uint16_t)frame[idx + 1] << 8);
    idx += 2;
    
    if(dst_addr_type == 0b10){
        dst_addr = frame[idx] | (frame[idx + 1] << 8);
        idx += 2;
    }else if(dst_addr_type == 0b11) {
        dst_addr = (uint64_t)frame[idx]              |
                    ((uint64_t)frame[idx + 1] << 8)  |
                    ((uint64_t)frame[idx + 2] << 16) |
                    ((uint64_t)frame[idx + 3] << 24) |
                    ((uint64_t)frame[idx + 4] << 32) |
                    ((uint64_t)frame[idx + 5] << 40) |
                    ((uint64_t)frame[idx + 6] << 48) |
                    ((uint64_t)frame[idx + 7] << 56);
        idx += 8;
    }

    src_panid  = frame[idx] | (frame[idx + 1] << 8);
    idx += 2;
    if(src_addr_type == 0b10){
        src_addr = frame[idx] | (frame[idx + 1] << 8);
        idx += 2;
    }else if(src_addr_type == 0b11) {
        src_addr = (uint64_t)frame[idx]              |
                    ((uint64_t)frame[idx + 1] << 8)  |
                    ((uint64_t)frame[idx + 2] << 16) |
                    ((uint64_t)frame[idx + 3] << 24) |
                    ((uint64_t)frame[idx + 4] << 32) |
    ...
    if(dst_addr_type == 0b10) ESP_EARLY_LOGI(TAG, "DST PANID %04x  DST ADDR %04llx", dst_panid, dst_addr);
    if(dst_addr_type == 0b11) ESP_EARLY_LOGI(TAG, "DST PANID %04x  DST ADDR %016llx", dst_panid, dst_addr);
    if(src_addr_type == 0b10) ESP_EARLY_LOGI(TAG, "SRC PANID %04x  SRC ADDR %04llx", src_panid, src_addr);
    if(src_addr_type == 0b11) ESP_EARLY_LOGI(TAG, "SRC PANID %04x  SRC ADDR %016llx", src_panid, src_addr);
}
